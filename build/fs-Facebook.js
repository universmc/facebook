...
async function executeAndMatchAIModels() {
  ...
  const match = {FACEBOOK: null, GROQ: null, GEMINI: null,HTML: null,CSS: null,CSS: null,JS: null,JSON: null,SVG: null,DRAWIO: null  }; // Initialize match results

  try {

    // Execute prompt --Facebook content generation task
    console.log("Starting Groq-html content generation task...");
    const FacebookCompletion = await groq.chat.completions.create({
      messages: [
    ...
    const FacebookContent = FacebookCompletion.choices[0]?.message?.content;
    const FacebookFilePath = `data/Facebook_${Date.now()}.md`;
    fs.writeFileSync(FacebookFilePath, FacebookContent || "No content generated.");
    console.log(`groq content saved to ${FacebookFilePath}`);
    match.FACEBOOK = FacebookContent ? "Success" : "Failure";
  ...
  console.log(`Documentation Generation: ${match.FACEBOOK}`);
